...
def export_audit_pack(root: Path, commit_id: str, output_dir: Path | None = None) -> Path:
    ...
    write_json(target / "motif_frame.json", state.get("motifFrame", {}))
    write_json(target / "reasoning_plan.json", state.get("reasoningPlan", {}))
    write_json(target / "reasoning_plans.json", {"plans": state.get("reasoningPlans", [])})
